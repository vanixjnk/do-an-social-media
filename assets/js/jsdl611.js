...
$(document).ready(function() {
    ...
    if (isValidDomain) {
        ...
        function changePage(page, title, url) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('a[data-page="' + page + '"]').parent('li.nav-item').addClass('active');
                    $('a.admin-route').removeClass('nuxt-link-exact-active');
                    $('a.admin-route[data-page="' + page + '"]').addClass('nuxt-link-exact-active');
                    $('li.profile-item').removeClass('active');
                    $('a[data-page="' + page + '"]').closest('li.profile-item').addClass('active');
                },
                error: function() {
        ...
        $(document).on('click', 'a[data-page]', function(e) {
            ...
            changePage(page, title, url);
        });

        $(window).on('popstate', function(e) {
            var state = e.originalEvent.state;
            if (state) {
                changePage(state.page, state.page.charAt(0).toUpperCase() + state.page.slice(1), '/' + state.page);
            } else {
                changePage('home', 'Trang chủ', '/');
            }
        });
